...
class HandeyeClient:
    def __init__(self, node: rclpy.node.Node, parameters: HandeyeCalibrationParameters):
        ...
        self.save_samples_client = self.node.create_client(ehm.srv.SaveSamples, hec.SAVE_CALIBRATION_TOPIC)
    ...
    def get_current_transforms(self):
        curr_tf = self.get_current_transforms_client.call(ehm.srv.TakeSample.Request()).samples.samples
        if len(curr_tf) == 0:
            return None
        return curr_tf[0]
    ...
    def save(self):
        self.save_samples_client.call(ehm.srv.SaveSamples.Request())
